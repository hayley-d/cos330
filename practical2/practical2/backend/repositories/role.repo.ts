...
export async function getRoleById(db: DB, roleId: UUID): Promise<Role | null> {
    const row = await db.get<RolesRow>(`SELECT * FROM roles WHERE role_id = ?`, [roleId]);
    return row ? roleFromRow(row) : null;
}

export async function getRoleByName(db: DB, name: string): Promise<Role | null> {
    const row = await db.get<RolesRow>(`SELECT * FROM roles WHERE role_name = ?`, [name]);
    return row ? roleFromRow(row) : null;
}

export async function listRoles(db: DB): Promise<Role[]> {
    const rows = await db.all<RolesRow>(`SELECT * FROM roles ORDER BY role_name ASC`);
...
export async function createRole(db: DB, dto: CreateRoleDTO): Promise<void> {
    await db.run(
     ...
     VALUES (?, ?, ?, ?)`,
        [dto.roleId, dto.name, dto.description ?? null, serializePermissions(dto.permissions)]
    );
}
...
export async function updateRole(
    db: DB,
    roleId: UUID,
    patch: { name?: string; description?: string | null; permissions?: CreateRoleDTO["permissions"] }
): Promise<void> {
    const sets: string[] = [];
    const vals: any[] = [];
    ...
    if (patch.description !== undefined) push("role_description", patch.description);
    if (patch.permissions !== undefined) push("permissions", serializePermissions(patch.permissions));
