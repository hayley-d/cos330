...
const server = http.createServer(
  async (req: IncomingMessage, res: ServerResponse) => {
    ...
    const session: SessionRow | null = sid
      ? (getSession(sid, ip, ua) as SessionRow | null)
      : null;
    ...
    if (req.method === "POST" && url.pathname === "/login") {
      ...
      if (
        !u ||
        !verifyPassword(body.password, u.password_salt, u.password_hash)
      ) {
    ...
    if (req.method === "POST" && url.pathname === "/upload") {
      ...
      if ((header(req, "x-csrf") ?? "") !== session?.csrf)
    ...
    if (req.method === "GET" && url.pathname === "/file") {
      ...
      const rec = db.prepare(`SELECT * FROM files WHERE id=?`).get(id) as
        | FileRow
        | undefined;

      if (!rec || rec.is_confidential)
        return json(res, 404, { error: "not_found" });
      if (!authorize(role, rec.kind, "read"))
        return json(res, 403, { error: "forbidden" });
    ...
    if (req.method === "GET" && url.pathname === "/confidential/read") {
      ...
      if (!rec || !rec.is_confidential)
        return json(res, 404, { error: "not_found" });
      if (!authorize(role, "confidential", "read"))
        return json(res, 403, { error: "forbidden" });
    ...
    if (req.method === "POST" && url.pathname === "/confidential/write") {
      ...
      if ((header(req, "x-csrf") ?? "") !== session?.csrf)
        return json(res, 403, { error: "csrf" });
      if (!authorize(role, "confidential", "write"))
        return json(res, 403, { error: "forbidden" });
      ...
      if (!verifyTotp(sec.secret_base32, String(totp)))
        return json(res, 401, { error: "bad_totp" });
