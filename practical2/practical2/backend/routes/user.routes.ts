...
import type {
    ...
    UserLoginDto,
    RequestUserOption,
    ValidateOtpDto, ApproveUserDto, UpdateUserRoleDto
} from "../types/user.types";
import {
    ApproveUserSchema,
    CreateUserDTOSchema, UpdateUserRoleSchema,
    UserLoginSchema,
    ValidateMfaSchema,
    ValidateOtpSchema
...
export default function userRoutes(db: DB) {
    ...
    router.post("/validate", async (req: Request<{}, {}, ValidateMfaDto>, res) => {
        try {
    ...
    router.post("/login", async (req: Request<{}, {}, UserLoginDto>, res) => {
        ...
        res.status(200).json({mfa_required: true, user_id: result.user?.userId, user_email: result.user?.email});
    });

    router.post("/validate-otp", async (req: Request<{}, {}, ValidateOtpDto>, res) => {
    ...
    router.patch("/approve", authMiddleware,  async (req : Request<{}, {}, ApproveUserDto>, res) => {
    ...
    router.patch("/role", authMiddleware,  async (req : Request<{}, {}, UpdateUserRoleDto>, res) => {
