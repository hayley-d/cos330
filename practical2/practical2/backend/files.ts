...
export function saveBinary(kind : Kind, userId : UUID, originalName : string, bodyBuf : Buffer) : UUID {
    ...
    my_database.prepare(`INSERT INTO files(id,kind,owner_user_id,original_name,stored_path,size,is_confidential,created_at,updated_at)
              VALUES(?,?,?,?,?,?,?,?,?)`)
...
export function readBinary(fileId : UUID) : { metadata: FileRow, buffer: Buffer } | null {
    const file : FileRow = my_database.prepare(`SELECT * FROM files WHERE id=?`).get(fileId);
...
export function createConfidential(userId : UUID, name : string, contentBuf : Buffer) : UUID {
    const id : UUID = randomUUID();
...
export function readConfidential(fileId : UUID)  : { metadata: FileRow, buffer: Buffer } | null {
    const file : FileRow = db.prepare(`SELECT * FROM files WHERE id=?`).get(fileId);

    if (!file || !file.is_confidential) return null;

    const metadata = db.prepare(`SELECT hkdf_salt,nonce,auth_tag FROM confidential_meta WHERE file_id=?`).get(fileId);

    const ct : Buffer = fs.readFileSync(f.stored_path);
    const pt = decConfidential(fileId, metadata.hkdf_salt, metadata.nonce, metadata.auth_tag, ct);
